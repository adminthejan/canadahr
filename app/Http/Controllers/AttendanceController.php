...
public function store(Request $request)
{
    // Manual form submission from the web UI
    if (!$request->isJson()) {
        $request->validate([
            'employee_id' => 'required',
            'date' => 'required|date',
        ...
        try {
            ...
            if (!$employee) {
                return redirect()->route('attendance.management')->with('error', 'Employee not found with the given ID.');
            }
